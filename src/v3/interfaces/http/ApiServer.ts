...
export function createApiServer(options: ApiServerOptions) {
  ...
  app.post('/api/v1/instances', requirePermission('instances:create'), async (req, res, next) => {
    try { res.status(201).json(await options.instances.create(createSchema.parse(req.body))) } catch (e) { next(e) }
  })
  app.get('/api/v1/instances/:id', requirePermission('instances:read'), async (req, res, next) => {
    try { res.json(await options.instances.get(idSchema.parse(req.params.id))) } catch (e) { next(e) }
  })
  app.patch('/api/v1/instances/:id', requirePermission('instances:update'), async (req, res, next) => {
    try { res.json(await options.instances.update(idSchema.parse(req.params.id), updateSchema.parse(req.body))) } catch (e) { next(e) }
  })
  app.delete('/api/v1/instances/:id', requirePermission('instances:delete'), async (req, res, next) => {
    try { await options.instances.delete(idSchema.parse(req.params.id)); res.status(204).send() } catch (e) { next(e) }
  })
