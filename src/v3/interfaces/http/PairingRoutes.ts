...
import { z } from 'zod'
import type { PairingRequest, PairingService } from '../../application/instances/PairingService'
...
const pairSchema = z.object({
  ...
  if (value.method === 'phone' && !value.phoneNumber) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['phoneNumber'], message: 'phoneNumber is required for phone pairing' })
  }
})
...
export function createPairingRoutes(pairing: PairingService, feed: LiveMessageFeed, requirePairPermission?: PermissionMiddleware): Router {
  ...
  router.post('/instances/:instanceId/pairing', requirePairPermission ?? ((_req: any, _res: any, next: any) => next()), async (req, res, next) => {
    try {
      const parsed = pairSchema.parse(req.body)
      const input: PairingRequest = {
